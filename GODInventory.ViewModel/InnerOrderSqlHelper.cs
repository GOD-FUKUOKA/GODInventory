...
{
    public class InnerOrderSqlHelper
    {
        ...
        public static IQueryable<t_innerorders> PendingOrderQuery(EntityDataSource entityDataSource1)
        {
            var q = from t_innerorders o in entityDataSource1.EntitySets["t_innerorders"]
...
ORDER BY o.受注日 desc, o.Status, o.transport_id,o.warehouse_id, o.県別, o.店舗コード, o.ＪＡＮコード,  o.伝票番号 LIMIT {1} OFFSET {2};";
        ...
        {
            //sqlStr = "SELECT t_innerorder.`出荷日`,t_innerorder.`納品日`,t_innerorder.`受注日`,t_innerorder.`店舗コード`," _
            //& " t_shoplist.`店名`,t_innerorder.`伝票番号`,t_innerorder.`口数`,t_innerorder.`ジャンル`,t_innerorder.`品名漢字`,t_innerorder.`規格名漢字`," _
            //& " t_innerorder.`発注数量`,t_innerorder.`実際配送担当`,t_shoplist.`県別`," _
            //& " t_innerorder.`発注形態名称漢字`,t_innerorder.`キャンセル`,t_innerorder.`ダブリ`,t_innerorder.`一旦保留`" _
            //& " FROM t_innerorder" _
            //& " INNER JOIN t_shoplist ON t_innerorder.`店舗コード` = t_shoplist.`店番`" _
            //& " WHERE t_innerorder.`キャンセル` = 'no' AND t_innerorder.`納品日` IS NULL" _
            //& " ORDER BY t_innerorder.`実際配送担当` ASC,t_shoplist.`県別` ASC,t_innerorder.`店舗コード` ASC," _
            //& " t_innerorder.`ＪＡＮコード` ASC,t_innerorder.`受注日` ASC,t_innerorder.`伝票番号` ASC"

            var q = (from t_innerorder o in entityDataSource1.EntitySets["t_innerorder"]
                     ...
                     orderby o.Status, o.実際配送担当, o.県別, o.店舗コード, o.商品コード, o.受注日, o.伝票番号
                     select new v_pendingorder
                     {
                         id受注データ = o.id受注データ,
                         出荷日 = o.出荷日,
        ...
        public static IQueryable<t_innerorders> ASNOrderSql(EntityDataSource entityDataSource1)
        {
            var q = (from t_innerorders o in entityDataSource1.EntitySets["t_innerorders"]
        ...
        public static IQueryable<t_innerorders> ShippedOrderSql(EntityDataSource entityDataSource1)
        {
            var q = (from t_innerorders o in entityDataSource1.EntitySets["t_innerorders"]
        ...
        public static List<t_innerorders> CanceledOrderSql(EntityDataSource entityDataSource1)
        {
            //由于entityDataSource1 缓存问题，所以使用新连接
            List<t_innerorders> list = null;
            using (GODDbContext ctx = new GODDbContext())
            {

                list = (from t_innerorders o in ctx.t_innerorders
        ...
        public static IQueryable<t_innerorders> ReceivedOrderSql(EntityDataSource entityDataSource1)
        {
            var q = (from t_innerorders o in entityDataSource1.EntitySets["t_innerorders"]
        ...
        public static List<v_pendingorder> ASNOrderDataListByMid(EntityDataSource entityDataSource1, long mid)
        {
            ...
            var orders = (from t_innerorders o in entityDataSource1.EntitySets["t_innerorders"]
                          join t_shoplist s in entityDataSource1.EntitySets["t_shoplist"] on o.店舗コード equals s.店番
                          where o.ASN管理連番 == mid
                          select new v_pendingorder {
                              ASN管理連番 = o.ASN管理連番,
        ...
        public static int ShippedOrderCount(EntityDataSource entityDataSource1) {
            int  count = (from t_innerorders o in entityDataSource1.EntitySets["t_innerorders"]
        ...
        public static int ASNCountByStore(short company_code, int store_code) {
            ...
            using (var ctx = new GODDbContext())
            {
                ...
                count = (from t_innerorders o in ctx.t_innerorders
        ...
        public static int CancelOrders(List<int> orderIds)
        {
            ...
            using (var ctx = new GODDbContext())
            {
                ...
                //string sqlFormat = "UPDATE t_innerorders SET `実際出荷数量`=0, `最終出荷数`=0, `納品原価金額`=0, `税額`=0, `備考`='キャンセル',`キャンセル`=@p1, `キャンセル時刻`=@p2 ,`Status`=@p3 WHERE `id受注データ` in ({0})";
                //string sql = String.Format(sqlFormat, String.Join(",", orderIds.ToArray()));
                //count = ctx.Database.ExecuteSqlCommand(sql, parameters);

                List<t_innerorders> orders = (from t_innerorders o in ctx.t_innerorders
                                            where orderIds.Contains(o.id受注データ)
                                            select o).ToList();
                foreach (var order in orders)
                {
                    InnerOrderSqlHelper.CancelOrder(ctx, order);
                }
        ...
        // 取消任意状态订单
        public static void CancelOrder(GODDbContext ctx, t_innerorders order)
        {

            var orderIds = new List<int> {  order.id受注データ };

            var orderList = new List<t_innerorders> { order };
            ...
            InnerOrderSqlHelper.AfterOrderQtyChanged(order, product);
            //当订单被取消掉的时候，重量=t_innerorders.`発注数量`* t_itemlist.`単品重量`。
            ...
            {
                InnerOrderSqlHelper.UpdateStockState(ctx, stockrecList);
            }
            
        }

        // 撤销取消掉订单
        public static void UncancelOrder(GODDbContext ctx, t_innerorders order)
        {
        ...
        public static int SendOrderToShipper(List<v_pendingorder> orders)
        {
            ...
            using (var ctx = new GODDbContext())
            {
                ...
                string sql = String.Format("UPDATE t_innerorders SET `一旦保留`=0,`配送担当受信`=@p1, `配送担当受信時刻`=@p2 ,`Status`=@p3 WHERE `id受注データ` in ({0})", String.Join(",", orderIds.ToArray()));
                count = ctx.Database.ExecuteSqlCommand(sql, parameters);
        ...
        public static int ShippingInfoConfirm(List<int> orderIds, DateTime ShippedAtDate, DateTime ReceivedAtDate, string shipNO)
        {
            ...
            using (var ctx = new GODDbContext())
            {
                ...
                string sql = String.Format("UPDATE t_innerorders SET `出荷日`=@p1, `納品日`=@p2,`Status`=@p3, `ShipNO`=@p4  WHERE `id受注データ` in ({0})", String.Join(",", orderIds.ToArray()));
                count = ctx.Database.ExecuteSqlCommand(sql, parameters);                
                ctx.SaveChanges();
            }
            return count;
        ...
        public static int UnShippingInfoConfirm(List<int> orderIds)
        {
            ...
            using (var ctx = new GODDbContext())
            {
                ...
                string sql = String.Format("UPDATE t_innerorders SET `出荷日`=NULL, `納品日`=NULL,`Status`={1}, `ShipNO`=NULL  WHERE `id受注データ` in ({0})", String.Join(",", orderIds.ToArray()), (int)OrderStatus.WaitToShip);
                count = ctx.Database.ExecuteSqlCommand(sql, parameters);
                ctx.SaveChanges();
            }
            return count;
        ...
        public static int UpdateOrderStatusShipped(List<string> shipNOs)
        {
            ...
            using (var ctx = new GODDbContext())
            {
                //var orderList = (from t_innerorders o in ctx.t_innerorders
                //                 where orderIds.Contains(o.id受注データ)
                //                 select o).ToList();

                string sql = string.Format("UPDATE t_innerorders SET `Status`={1}  WHERE `shipNO` in ({0})", String.Join(",", shipNOs.Select(s => "'" + s + "'").ToArray()), (int)OrderStatus.Shipped);
        ...
        public static int UpdateOrderreportState(List<string> shipNOs,int status)
        {
            ...
            using (var ctx = new GODDbContext())
            {
                ...
                //string sql = String.Format("UPDATE t_innerorders SET `reportState`={0} WHERE `ShipNO`={1}", status, shipNOs);

                string sql = string.Format("UPDATE t_innerorders SET `reportState`={1}  WHERE `shipNO` in ({0})", String.Join(",", shipNOs.Select(s => "'" + s + "'").ToArray()), status);
                count = ctx.Database.ExecuteSqlCommand(sql);
        ...
        public static int FinishOrders(List<int> orderIds)
        {
            ...
            using (var ctx = new GODDbContext())
            {
                ...
                string sql = String.Format("UPDATE t_innerorders SET `受領確認`=1, `Status`=@p3  WHERE `id受注データ` in ({0})", String.Join(",", orderIds.ToArray()));
                count = ctx.Database.ExecuteSqlCommand(sql, parameters);
                ctx.SaveChanges();
            }
            return count;
        ...
        public static int LockOrdersByShipNO(string shipNO)
        {
            ...
            using (var ctx = new GODDbContext())
            {
                string sql = "UPDATE t_innerorders SET `Status`={2}  WHERE `ShipNO` = ({0}) && `Status`={1}";
        ...
        public static int GenerateOrderChuHeNO(List<string> shipNOs) {
            ...
            using (var ctx = new GODDbContext())
            {
                ...
                var orders = (from t_innerorders o in ctx.t_innerorders
                ...
                foreach (var gos in grouped_orders)
                {
                    ...
                    var sql1 = String.Format("UPDATE t_innerorders SET  `出荷No`={1}, `Status`={2}  WHERE  `id受注データ` in ({0}) ", String.Join(",", oids.ToArray()), chuhe_no, (int)OrderStatus.ASN);
        ...
        public static long GenerateASNByShipNOs(GODDbContext ctx, List<string> shipNOs)
        {
            ...
            string sql1 = String.Format("UPDATE t_innerorders SET `ASN管理連番`={1}  WHERE `ShipNO` in ({0}) ", String.Join(",", shipNOs.Select(s => "'" + s + "'").ToArray()), mid);
        ...
        public static long GenerateASNByOrderIds(GODDbContext ctx, List<int> orderIds)
        {
            ...
            long mid = EDITxtHandler.GenerateMID(ctx);
            // 无需 条件OrderStatus.ASN, 取消的订单也要生成ASN
            var sql1 = String.Format("UPDATE t_innerorders SET `ASN管理連番`={1}  WHERE `id受注データ` in ({0}) ", String.Join(",", orderIds.ToArray()), mid);
            var path = EDITxtHandler.BuildASNFilePath(mid);
        ...
        public static List<t_itemlist> ASNstockrecDataListByMid(EntityDataSource entityDataSource1, short? mid)
        {
            ...
            //var orders = (from t_itemlist o in entityDataSource1.EntitySets["t_innerorders"]
            //              join t_shoplist s in entityDataSource1.EntitySets["t_shoplist"] on o.店舗コード equals s.店番
            //              where o.ASN管理連番 == mid
            //              select new v_pendingorder
            //              {
        ...
        public static int NotifyShipper(GODDbContext ctx, List<v_pendingorder> pendingOrders, string transportName)
        {
            ...
            string sql = "SELECT MAX(t_innerorders.`社内伝番`) FROM t_innerorders";
            ...
            sql = String.Format("UPDATE t_innerorders SET `Status`={3}, `最終出荷数`=`実際出荷数量`, `配送担当受信`=TRUE, `配送担当受信時刻`= NOW() WHERE `Status`={0} AND `実際配送担当`='{1}' AND `id受注データ` in ({2});", (int)OrderStatus.NotifyShipper, transportName, String.Join(",", orderIds.ToArray()), (int)OrderStatus.WaitToShip);
        ...
        public static List<t_innerorders> OrderListByShipNO(EntityDataSource entityDataSource1, string shipNO)
        {
        ...
        public static int DeleteFaxOrder(int orderId)
        {
            ...
            using (var ctx = new GODDbContext())
            {
                string sql = "DELETE  from t_innerorders WHERE `id受注データ`={0} AND `入力区分`=1;";
        ...
        public static void AfterOrderQtyChanged(t_innerorders order, t_itemlist item)
        {
        ...
        public static void ChangeOrderStatusToPending(GODDbContext ctx, t_innerorders order)
        {
            order.実際出荷数量 = order.発注数量;
            order.納品口数 = order.口数;
            order.ダブリ = "no";
        ...
        public static int ComputeFreight(t_innerorders order, decimal unitfee = 0, string columnname = null) 
        {
            ...
            if (columnname != null)
            {
                fee = InnerOrderHelper.ComputeFreight(order, unitfee, columnname);
